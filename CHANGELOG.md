...
- Unauthenticated MSX playback requests no longer attempt user impersonation.

### Security

- All token-bearing playlist routes reject cross-site browser requests.
...
- Native seeks while paused now update Music Assistant and resume position reporting correctly.
- Shared-stream catch-up and subscriber buffers are bounded to roughly 600 KB each.

## [1.5.7] - 2026-09-02

### Fixed

- Native MSX seeks now update Music Assistant elapsed time.

## [1.5.6] - 2026-09-02

### Fixed

- Local proxy and shared-group streams use Music Assistant's gapless-burst output pacing.

## [1.5.5] - 2026-09-02

### Fixed

- Native MSX pause/resume now reaches Music Assistant.
- HTTP play-context restores a player marked unavailable after a WebSocket drop.
- Queue-loading and audio-prep failures return MSX-safe errors instead of HTTP 500.
- Artist search results keep their source provider.
...
- Browser WebSocket control connections now require a same-origin handshake, while native Origin-less MSX clients remain supported.
- Playback control routes now reject same-site cross-origin browser requests in addition to cross-site requests.

## [1.5.2] - 2026-09-01

### Security

- Concurrent Party cover fetch/decode/render work is bounded to prevent memory exhaustion from many unique cover requests.

## [1.5.1] - 2026-09-01

### Fixed

- Party status documentation now matches its narrowed expected-error fallback contract.
...
- MSX player tests now satisfy the upstream Music Assistant mypy gate without changing runtime behavior.
...
- Group member playback now releases stale live-source sessions through Music Assistant's internal player handlers.
...
- Grouped TVs that use a different output format no longer receive the leader's encoded audio advertised as the wrong type.

## [1.4.21] - 2026-08-27

### Fixed

- Search on Media Station X older than 0.1.155 no longer shows "Decompression not supported" after a query is entered.

## [1.4.20] - 2026-08-27

### Fixed

- Queue items that only carry their URI on the queue entry itself can still be played.
- Party cover stamping no longer changes Pillow's process-wide image size limit.
...
- Party cover stamping no longer fetches images from a host supplied in the request.
- Long Music Assistant queues keep the current track first on the TV playlist.
- Seeking on the TV before the first progress report is accepted.
- Ending a stream no longer waits forever when a TV has already disconnected.
...
- Standalone and inlined test harnesses now use their native repository layouts, keeping provider release synchronization independent of runner-specific paths.
...
- Provider and per-TV configuration now use Music Assistant's live-instance options contract introduced in upstream [#5017](https://github.com/music-assistant/server/pull/5017), keeping setup and reconfiguration compatible with current servers.
